...
ggplot(elbo.df, aes(x = method, y = elbo)) + 
  geom_point() +
  scale_x_discrete(limits = c("No size factors",
                              "Library size normalization", 
                              "scran size factors")) +
  labs(x = NULL, y = "ELBO")
```
...
The distribution of $p$-values is very similar across methods, but the small differences confirm the above ranking: `scran` does best, while library-size normalization does worst.
...
ggplot(KL.df, aes(x = fit, y = KL)) +
  ...
  scale_x_discrete(limits = c("noscale", "libsize", "scran")) +
...
Factors are very similar across methods. I don't see anything worth commenting on here.
