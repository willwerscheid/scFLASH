...
sim_data <- function(n = 100, p = 200, K = 6, L.nn = 10, F.nn = 20, se = 0.1, seed = 666) {
  set.seed(seed)
  ...
  for (k in 2:K) {
    L.nn.idx <- seq((k - 2) * L.nn + 1, (k - 1) * L.nn)
    F.nn.idx <- seq((k - 2) * (F.nn / 2) + 1, k * (F.nn / 2))
...
Not only is the SNMF basically nonnegative, but it does a better job recovering the true factors than NMF!
...
The SNMF recovers the correct number of factors, which NMF is of course unable to do.

Finally I make the simulation scenario a bit more difficult (more noise, less sparsity) and try again with the correct number of factors:

```{r sim2}
sim2 <- sim_data(K = 6, se = 1, L.nn = 20, F.nn = 40)

nnmf_res2 <- NNLM::nnmf(sim2$Y, k = 6, verbose = 0)
...
SNMF again does quite a bit better!
