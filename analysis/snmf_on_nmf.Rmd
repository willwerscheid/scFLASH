...
SNMF (almost) recovers the correct number of factors, which NMF is of course unable to do. If, however, I initialize EBMF using the NMF fit and then backfit using point-exponential priors, I get a better NMF fit:

```{r sim1_nnebmf}
nnmf_res1c <- flash.init(sim1$Y) %>%
  flash.set.verbose(0) %>%
  flash.init.factors(
    list(nnmf_res1b$W, t(nnmf_res1b$H)),
    ebnm.fn = ebnm::ebnm_point_exponential
  ) %>%
  flash.backfit(maxiter = 200) %>%
  flash.nullcheck() %>%
  flash.backfit() %>%
  flash.nullcheck()
