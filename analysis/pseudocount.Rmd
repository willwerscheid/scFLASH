...
```{r elbo_breakdown}
KL <- sapply(fl.list, function(fl) sum(unlist(fl$fit$KL)))
...
One can instead calculate the Spearman rank correlation for each choice of $\alpha$. We want small predictions to correspond to small counts, and large predictions for large counts. The Spearman correlation will capture this. These results suggest that setting $\alpha$ to be something between 0.5 and 1 is best. 
...
for (i in 1:length(fl.list)) {
  ct <- pseudocounts[i]
  preds <- ct * exp(flashier:::lowrank.expand(fl.list[[i]]$fit$EF)) - ct
...
log1p.LL <- fl.list[[log1p.idx]]$loadings$normalized.loadings[[1]]
factor.idx <- lapply(fl.list, function(fl) {
  corr <- abs(crossprod(fl$loadings$normalized.loadings[[1]], log1p.LL))
...
for (k in c(2, 5, 8)) {
  ...
  LL <- mapply(FUN = function(fl, match.k) {
    if (length(match.k) > 0) {
      return(fl$loadings$normalized.loadings[[1]][, match.k])
    } else {
      return(NULL)
    }
...
all.lme <- lapply(fl.list, function(fl) {
  apply(fl$loadings$normalized.loadings[[1]], 2, function(x) {
    sum(x^2 * log.mean.exp)})
})
# Sparsity.
all.sprs <- lapply(fl.list, function(fl) {
  sapply(fl$fit$g, function(g) g[[1]]$pi0)
})

lme.mat <- mapply(function(idx, lme) {
  lme[sapply(1:log1p.K, function(k) {
    if (k %in% idx) which(k == idx) else NA
  })]
}, factor.idx, all.lme)
