...
n <- ncol(Deng$fl.dat)
min.sd <- sd(log1p(rpois(1e7, 1/n) / Deng$pc / median(Deng$size.factors)))
...
K <- 20
t0 <- Sys.time()
snmf.fl <- flash.init(Deng$fl.dat, S = min.sd, var.type = 1) %>%
  flash.init.factors(
    list(matrix(rowMeans(Deng$fl.dat), ncol = 1), matrix(1, nrow = ncol(Deng$fl.dat), ncol = 1)),
...
t1 <- Sys.time()
snmf.t <- t1 - t0
saveRDS(list(fl = snmf.fl, t = snmf.t), "./output/deng/smnf.rds")


t0 <- Sys.time()
greedy.fl <- flash.init(Deng$fl.dat, S = min.sd, var.type = 1) %>%
  flash.init.factors(
    list(matrix(rowMeans(Deng$fl.dat), ncol = 1), matrix(1, nrow = ncol(Deng$fl.dat), ncol = 1)),
...
t1 <- Sys.time()
greedy.t <- t1 - t0
saveRDS(list(fl = greedy.fl, t = greedy.t), "./output/deng/greedy.rds")
...
t1 <- Sys.time()
bf.t <- t1 - t0
saveRDS(list(fl = bf.fl, t = bf.t), "./output/deng/bf.rds")


t0 <- Sys.time()
greedy.fl2 <- bf.fl %>%
  flash.add.greedy(
    Kmax = K - bf.fl$n.factors,
    ebnm.fn = ebnm::ebnm_point_exponential,
    init.fn = function(f) init.fn.default(f, dim.signs = c(1, 1))
  )
t1 <- Sys.time()
greedy.t2 <- t1 - t0
saveRDS(list(fl = greedy.fl2, t = greedy.t2), "./output/deng/greedy2.rds")
...
t1 <- Sys.time()
bf.t2 <- t1 - t0
saveRDS(list(fl = bf.fl2, t = bf.t2), "./output/deng/bf2.rds")
...
nnmf.res <- NNLM::nnmf(
  Deng$fl.dat,
  init = list(H0 = matrix(1, nrow = 1, ncol = ncol(Deng$fl.dat))),
  k = K - 1,
  verbose = 0
)
nnlm.fl <- flash.init(Deng$fl.dat, S = min.sd, var.type = 1) %>%
  flash.set.verbose(0) %>%
  flash.init.factors(
...
t1 <- Sys.time()
nnlm.t <- t1 - t0
saveRDS(list(fl = nnlm.fl, t = nnlm.t), "./output/deng/nnlm.rds")
...
t1 <- Sys.time()
nnlmbf.t <- t1 - t0
saveRDS(list(fl = nnlmbf.fl, t = nnlmbf.t), "./output/deng/nnlmbf.rds")


t0 <- Sys.time()
nzpe.fl <- flash.init(Deng$fl.dat, S = min.sd, var.type = 1) %>%
  flash.set.verbose(0) %>%
  flash.init.factors(
    list(nnmf.res$W[, c(K, 1:(K-1))], t(nnmf.res$H[c(K, 1:(K-1)), ])),
    ebnm.fn = c(
...
t1 <- Sys.time()
nzpe.t <- t1 - t0
saveRDS(list(fl = nzpe.fl, t = nzpe.t), "./output/deng/nzpe.rds")
...
nzpe.fl2 <- flash.init(Deng$fl.dat, S = min.sd, var.type = 1) %>%
  flash.set.verbose(0) %>%
  flash.init.factors(
...
t1 <- Sys.time()
nzpe.t2 <- t1 - t0
saveRDS(list(fl = nzpe.fl2, t = nzpe.t2), "./output/deng/nzpe2.rds")
